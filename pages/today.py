...
def router():
    @ui.page('/today')
    async def today_summary(client: Client):
        with ui.column().classes('p-4 sm:p-8 w-full max-w-[375px] mx-auto items-center'):
            ui.label('Today Summary').classes('text-h4 font-bold mb-4')

            async def print_to_hardware(e):
                e.sender.disable()
                e.sender.text = 'Printing...'
                ui.notify("Sending to physical printer...", type='info')
                await asyncio.sleep(0.2)
                
                try:
                    physical_result = await asyncio.to_thread(
                        subprocess.run,
                        ['python3', '/home/ncarigi/print_manager.py', '--real'],
                        capture_output=True,
                        text=True
                    )
            ...
            ui.button('🖨️ Print', on_click=print_to_hardware, color='green').classes('w-full max-w-sm mt-4 font-bold py-3 shadow-md')
            ...
            with receipt_container:
                ...
                ui.label('Fetching APIs...').classes('mt-2 text-gray-500')
            ...
            try:
                result = await asyncio.to_thread(
                    subprocess.run,
                    ['python3', '/home/ncarigi/print_manager.py'],
                    capture_output=True,
                    text=True
                )
